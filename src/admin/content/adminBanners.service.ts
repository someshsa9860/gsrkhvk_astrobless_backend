...
import { keyToUrl } from '../../lib/storage/index.js';
...
function withImageUrl<T extends { imageKey: string }>(row: T) {
  return { ...row, imageUrl: keyToUrl(row.imageKey) };
}
...
export async function listBanners(q: BannerListQuery) {
  ...
  return { items: items.map(withImageUrl), page: q.page ?? 1, limit, total, totalPages: Math.ceil(total / limit) };
}
...
export async function listPublicBanners(placement: string) {
  const now = new Date();
  const rows = await prisma.banner.findMany({
    where: { placement, isActive: true, startsAt: { lte: now }, endsAt: { gte: now } },
