...
export const CreateAstrologerSchema = z
  .object({
    ...
    pricePerMinChat: z.number().int().min(100).describe('Chat rate — paise per minute'),
    pricePerMinCall: z.number().int().min(100).describe('Voice-call rate — paise per minute'),
    pricePerMinVideo: z.number().int().min(100).describe('Video-call rate — paise per minute'),
    ...
    pricePerReport: z.number().int().min(0).optional().describe('Kundli report rate — paise'),
...
export const UpdateAstrologerSchema = z.object({
  ...
  kycDocsRef: z.object({
    ...
    aadhaarBackDocUrl: z.string().optional(),
    panDocUrl: z.string().optional(),
    selfieDocUrl: z.string().optional(),
    certificationDocUrl: z.string().optional(),
