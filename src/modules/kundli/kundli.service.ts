...
import { kundliProfiles } from '../../db/schema/content.js';
...
export async function listProfiles(customerId: string) {
  return db.query.kundliProfiles.findMany({
    where: eq(kundliProfiles.customerId, customerId),
    orderBy: (t, { desc }) => [desc(t.createdAt)],
  });
}

export async function getProfile(customerId: string, profileId: string) {
  const profile = await db.query.kundliProfiles.findFirst({
    where: and(eq(kundliProfiles.id, profileId), eq(kundliProfiles.customerId, customerId)),
  });
...
export async function createProfile(
  ...
  const [profile] = await db.insert(kundliProfiles).values({
    customerId,
    label:           input.label,
...
export async function deleteProfile(customerId: string, profileId: string) {
  ...
  await db.delete(kundliProfiles).where(eq(kundliProfiles.id, profileId));
...
function filterPreBirthDasha(dasha: DashaPeriod[], birthDate: string): DashaPeriod[] {
  return dasha
    .filter((period) => period.endDate > birthDate)  // keep periods that haven't ended before birth
    .map((period) => ({
      ...period,
...
export async function getReport(customerId: string, profileId: string) {
  ...
  if (!chartData || !profile.chartComputedAt) {
    ...
    await db.update(kundliProfiles)
      .set({ chartData, chartComputedAt: new Date(), updatedAt: new Date() })
      .where(eq(kundliProfiles.id, profileId));
  ...
  // Apply pre-birth dasha filter based on admin setting
