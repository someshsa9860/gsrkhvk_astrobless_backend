...
export class RazorpayProvider implements PaymentProvider {
  ...
  async createOrder(input: CreateOrderInput): Promise<CreateOrderResult> {
    try {
      ...
      return {
        ...
        clientPayload: { orderId: res.data.id, keyId: env.RAZORPAY_KEY_ID, amount: input.amountPaise, currency: input.currency },
  ...
  async refund(providerPaymentId: string, amountPaise: number, idempotencyKey: string): Promise<ProviderWebhookEvent> {
    try {
      const res = await axios.post(
      ...
      return { eventType: 'refundProcessed', providerPaymentId, amountPaise, raw: res.data };
    } catch {
...
function parseRazorpayEvent(payload: Record<string, unknown>): ProviderWebhookEvent {
  ...
  return {
    ...
    amountPaise: entity['amount'] as number | undefined,
    raw: payload,
  };
}
